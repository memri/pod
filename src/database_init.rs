...
lazy_static! {
    pub static ref BOOL_COLUMNS: HashSet<String> = { database_column_type().into_iter().collect() };
}

fn database_column_type() -> Vec<String> {
    let mut bool_columns = vec![];
    ...
    for column in columns.into_iter() {
        if column.dbtype == DatabaseColumnType::Bool {
            bool_columns.push((*column.name).to_string());
        }
    }
    bool_columns.push("deleted".to_string());
    bool_columns
}
