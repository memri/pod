...
pub fn init(sqlite: &Pool<SqliteConnectionManager>) {
    ...
    conn.execute_batch(EDGES_TALBLE_DDL)
        .unwrap_or_else(|err| panic!("Failed to create edges table, {}", err));
    conn.execute_batch(ITEMS_INDEX_DDL)
        .unwrap_or_else(|err| panic!("Failed to create items indexes, {}", err));
    conn.execute_batch(EDGE_INDEXES_DDL)
        .unwrap_or_else(|err| panic!("Failed to create edges indexes, {}", err));
...
const EDGES_TALBLE_DDL: &str = "CREATE TABLE IF NOT EXISTS edges (
